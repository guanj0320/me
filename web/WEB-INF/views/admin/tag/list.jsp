    ...
    <script>
        $(document).ready(function(){
            ...
            $('#btnQuery').click(function(){
                $('#tagForm').attr("action","${ctx}/admin/tag/list");
                $('#tagForm').submit();
            });

            $('#btnAdd').click(function(){
                location.href = "${ctx}/admin/tag/add";
            });

        });
        function doUpdate(id) {
            location.href = "${ctx}/admin/tag/modify?id="+id;
        }
        ...
        function doDelete(id) {
            ...
            $('#delModal').modal('show').on('shown.bs.modal',function(){
                ...
                $('#delBtn').click(function(){
                    $('#tagForm').attr("action","${ctx}/admin/tag/remove");
                    $('#tagForm').submit();
                });
            });
        }
        ...
        <table class="table table-striped table-hover">
            ...
            <c:forEach items="${tags}" var="tag" varStatus="status">
                <tr>
                    ...
                    <td>${tag.tagname}</td>
                    <td>${tag.num}</td>
                    <td><fmt:formatDate value="${tag.createtime}" type="both"/></td>
                    <td>
                        <a href="#" onclick="doUpdate('${tag.tid}')" title="打开"><i class="glyphicon glyphicon-folder-open"></i></a>
                        <a href="#" onclick="doDelete('${tag.tid}')" title="删除"><i class="glyphicon glyphicon-trash"></i></a>
                    </td>
                </tr>
            </c:forEach>
            </tbody>
